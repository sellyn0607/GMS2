...
<body>
	<div id="user">
		...
		<form id="userz">
			...
			<button id="overButt">아이디중복확인</button>
		</form>
		<br> 비밀번호 : <input type="password" name="pw" maxlength="20"
			size="12"> <br> 이름 : <input type="text" name="name"
			maxlength="5" size="8"><br> 명단 : <input type="radio"
			name="roll" value="팀장" />팀장 <input type="radio" name="roll"
			value="팀원" />팀원<br> 팀ID : <select name="team_id"><option
	 ...
	 document.getElementById('joinButt').addEventListener(
		...
		document
				...
				.addEventListener(
						'click',
						function() {
							var form2 = document.getElementById('userz');
							form2.action = "${context}/member.do";
							var node = document.createElement('input');
							...
							//location.href='${context}/member.do?action=findid&page=index&userid='+document.joinForm.userid.value;
							alert('${over}');
							if ('${over}' === 'FALSE') {
